...
export default function Filters({ filter, onChange, projects, projectsWithDerived, filteredCount }) {
  ...
  const activeFilterText = useMemo(() => {
    const parts = []
    if (filter.status !== 'all') parts.push(STATUS_LABELS[filter.status])
    if (filter.risk !== 'all') parts.push(filter.risk === 'on-track' ? 'On Track' : 'At Risk')
      ...
      <div className="filters-header">
        ...
        >
          <div className="filters-title">
            ...
            {activeFilterText && (
              <span className="filters-active-badge">{activeFilterText}</span>
            )}
          </div>
          <svg
            className={`filters-chevron ${isExpanded ? 'filters-chevron-up' : ''}`}
            width="16"
            height="16"
            viewBox="0 0 16 16"
            fill="none"
